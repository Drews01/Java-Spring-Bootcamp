...
public class DynamicAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
    ...
    public AuthorizationDecision authorize(Supplier<? extends Authentication> authentication,
        ...
        Set<Long> allowedMenuIds = roleMenuRepository.findByRole_NameIn(roleNames).stream()
                .map(com.example.demo.entity.RoleMenu::getMenuId)
                .collect(Collectors.toSet());

        // Find all menus with URL patterns
        List<com.example.demo.entity.Menu> menusWithPatterns = menuRepository.findAll().stream()
