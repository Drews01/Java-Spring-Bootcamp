...
/**
 * Utility class to create {@link ResponseEntity} instances that wrap the
 * standardized {@link ApiResponse}. Consolidates common response patterns so
 * controllers stay lean and focused on business logic.
 */
public final class ResponseUtil {
     ...
     */
    public static <T> ResponseEntity<ApiResponse<T>> error(HttpStatus status, String message, ErrorDetails details) {
        return ResponseEntity.status(status)
                .body(ApiResponse.error(message, details, status.value()));
    }
