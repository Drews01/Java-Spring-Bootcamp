...
public interface ProductRepository extends JpaRepository<Product, Long> {
  ...
  @Query(
      "SELECT p FROM Product p WHERE p.code = :code AND (p.deleted = false OR p.deleted IS NULL)")
  ...
  @Query(
      "SELECT p FROM Product p WHERE p.isActive = true AND (p.deleted = false OR p.deleted IS NULL)")
