...
public interface LoanApplicationRepository extends JpaRepository<LoanApplication, Long> {
    ...
    @Query("SELECT COALESCE(SUM(la.amount), 0) FROM LoanApplication la "
    ...
    @Query("SELECT COALESCE(SUM(la.amount), 0) FROM LoanApplication la WHERE la.currentStatus = :status")
    Double sumAmountByCurrentStatus(@Param("status") String status);

    @Query("SELECT COALESCE(SUM(la.totalAmountToPay), 0) FROM LoanApplication la WHERE"
    ...
    @Query("SELECT COUNT(la) FROM LoanApplication la WHERE la.currentStatus = :status AND YEAR(la.createdAt) = :year")
    Long countByCurrentStatusAndYear(
            @Param("status") String currentStatus, @Param("year") int year);

    @Query("SELECT COALESCE(SUM(la.amount), 0) FROM LoanApplication la WHERE la.currentStatus = :status AND YEAR(la.createdAt) = :year")
    Double sumAmountByCurrentStatusAndYear(
